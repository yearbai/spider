...
public class MyRedisScheduler extends DuplicateRemovedScheduler implements MonitorableScheduler, DuplicateRemover {
    ...
    private final static Logger LOG = LogManager.getLogger(MyRedisScheduler.class);
...
//            jedis.del( this.getSetKey( task ) );
            jedis.del( CodeConstant.FINISH_LIST );
        } finally {
    ...
    public boolean isDuplicate(Request request, Task task) {
        ...
        try {
            isDuplicate = jedis.sismember( CodeConstant.FINISH_LIST , request.getUrl() ).booleanValue();
            System.out.println( "是否重复 " + isDuplicate );
        } finally {
            this.pool.returnResource( jedis );
        }
