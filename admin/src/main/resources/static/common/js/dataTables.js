...
//'use strict';
    ...
    window.CommonTable = function (tableId, queryId, searchDiv, config) {
        ...
        // 初始化表格
        this.initTable(tableId, queryId, searchDiv, config);
     ...
     * 初始化表格
     */
    CommonTable.prototype.initTable = function (tableId, queryId, searchDiv, config) {
        this.data = this.getServerData(config.ajaxSource, null, tableId);
        ...
        var allowPaging = "true";//this.data.query.allowPaging;
        ...
        this.table = $('#' + tableId).DataTable($.extend({
            ...
            "language": { // 中文支持
                "sUrl": ctxPath + "/common/json/zh_CN.json"
            },
            "displayLength": that.data.pageData.size,// 每页记录条数，默认为10
            "serverSide": true,
            "ajaxDataProp": "data",
        ...
        if (this.customButton) {
            this.customButton.click(function () {
                ...
                modals.openWin({
                    ...
                    width: '400px',
                    url: ctxPath + "/query/tableConfig?queryId=" + that.data.query.id + "&pageName=" + pagename,
                    hideFunc: function () {
                        that.setVisible();
                    }
                });
...
// isCondition默认为true;likeOption默认false 即不拼接%
     ...
     *            table的ID
     */
    CommonTable.prototype.getServerData = function (url, pageInfo, tableId) {
        var dataCache = $("#dataCache" + tableId);
        ...
        if(typeof (this.config.columns) == "undefined") {
            ...
            for (var j = 0; j < columns.length; j++) {
                ...
                for (var i = 0; i < retData.pageData.records.length; i++) {
                    retData.pageData.records[i]["rowIndex"] = start + i + 1;
                    //获取关联对象的值如message.sendSubject
                    var value_str = "retData.pageData.records[i]." + column.key;
                    ...
                    if (column.enableTooltip) {
                        ...
                        //重构为支持关联对象方式
                        var title = eval("retData.pageData.records[i]." + (column.tooltip || column.key));
                        var maxLen = parseInt(column.maxLen || 20);
                    ...
                    if (column.render) {
                        ...
                        var columnRender = this.getRenderValueByMatcher(column.render, retData.records[i]);
                        ...
                        if (value != null) {
                            if (obj.type == "eq") {
                                ...
                                obj.url = ctxPath + obj.url;
                                //retData.rows[i][column.key] = "<a href='#' onclick='javascript:modals.openWin(" + JSON.stringify(obj) + ")'>" + retData.rows[i][column.key] + "</a>";
                                eval(value_str + "=\"<a href='#' onclick='javascript:modals.openWin(\" + JSON.stringify(obj) + \")'>\" + retData.pageData.records[i][column.key] + \"</a>\"");
                                value = eval(value_str);
                                ...
                                if (obj.params) {
                                    ...
                                    if (retData.pageData.records[i][retData.query.key]) {
                                        //invoke_str = obj.method + "('" + retData.rows[i][retData.query.key] + "')";
                                        var value_key = eval("retData.pageData.records[i]." + retData.query.key);
    ...
    CommonTable.prototype.getCustomHiddenColumns = function (queryId, pageName) {
        ...
        if (!pageName)
            return retData;
        ajaxPost(ctxPath + "/query/getSelectedColumns", {queryId: queryId, pageName: pageName}, function (hideCols) {
            retData = hideCols;
        });
        return retData;
    }
...
//匹配出[name]类似的字符串，并替换真实的值
    ...
    CommonTable.prototype.fillDataTable = function (sSource, aoData, fnCallback, oSettings) {
        ...
        if (this.loaded) {// 换页
            ...
            result = this.getServerData(sSource,pageInfo, oSettings.sTableId);
            this.data = result;
        ...
        var obj = {};
        obj['data'] = result.pageData.records;//result.rows;
        obj["iTotalRecords"] = result.pageData.total;// result.pageInfo.count;
        obj["iTotalDisplayRecords"] = result.pageData.total;// result.pageInfo.count;
        fnCallback(obj);
        //序号排序
